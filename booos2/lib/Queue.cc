...
void Queue::insert(Queue::Element * elem) {
	...
	for(aux=this->head()->next();aux->next()!=this->head()->next();aux=aux->next()){
	...
	//Loop para descobrir qual o ultimo elemento. **
	//for(aux=this->head()->next();aux->next()!=this->head()->next();aux=aux->next());
...
void Queue:: insert_ordered(Element* elem){
	...
	for(ant=this->head()->next();ant->next()!=this->head()->next();ant=ant->next()){
	...
	int c;
    for (prox=this->_head.next(),c=0; prox != this->_head.next() || c==0;prox=prox->next(),c++){
