...
//	this->print_queue(this->__ready);
	...
	if(this->_state==READY){
		...
		else if(BOOOS_Configuration::SCHEDULER_TYPE==Scheduler::SCHED_PRIORITY){
			Task::__ready.insert_ordered(this);
			}
	...
	t->__actn++;
	t->t1=Timer::time();
	if(s==Scheduler::FINISHING){
		Task::_Task_Count--;
		cout << "Task " << this->_tid << " exit: response time " << (Timer::time()-this->__time_init)/1000 << " ms, CPU time " << this->__time_exec/1000 << " ms, " << this->__actn << " activations" << endl;
	}
...
void Task::exit(int code) {
	...
	while(this->__pending.length()){
		...
		aux->_state=READY;
		if(BOOOS_Configuration::SCHEDULER_TYPE==Scheduler::SCHED_FCFS){
					Task::__ready.insert(aux);
				}
		else if(BOOOS_Configuration::SCHEDULER_TYPE==Scheduler::SCHED_PRIORITY){
					Task::__ready.insert_ordered(aux);
					}
